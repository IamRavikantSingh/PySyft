...
class Decryptor:
    ...
    def __init__(self, context, secret_key):
        ...
        self.secret_key_array = [secret_key.data]
        ...
        for j in range(1, len(encrypted)):
            for i in range(len(coeff_modulus)):
                phase[i] = poly_add_mod(
                    poly_mul_mod(
                        encrypted[j][i], secret_key_array[j - 1][i], coeff_modulus[i], coeff_count,
                    ),
                    phase[i],
                    coeff_modulus[i],
                    coeff_count,
    ...
    def _get_sufficient_sk_power(self, max_power):
        ...
        if max_power == len(self.secret_key_array):
            return self.secret_key_array

        while len(self.secret_key_array) < max_power:
            sk_extra_power = [0] * len(coeff_modulus)
            for i in range(len(coeff_modulus)):
                sk_extra_power[i] = poly_mul_mod(
                    self.secret_key_array[-1][i],
                    self.secret_key_array[0][i],
                    coeff_modulus[i],
                    coeff_count,
                )
            self.secret_key_array.append(sk_extra_power)
